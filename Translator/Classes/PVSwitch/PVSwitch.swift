...
open class PVSwitch: UIControl {
    ...
    @IBInspectable var thumbOffTintColor: UIColor = .white {
        didSet {
            self.configureThumbColors()
        }
    }
    ...
    @IBInspectable var trackOffTintColor: UIColor = .black.withAlphaComponent(0.34) {
        didSet {
            self.configureTrackColors()
        }
    }
    ...
    private func commonInit(with frame: CGRect = CGRect(x: 0, y: 0, width: 58, height: 20)) {
...
private extension PVSwitch {
    ...
    private func configureThumb(frame: CGRect) {
        ...
        self.switchThumb.layer.shadowOffset = CGSize(width: 0.0, height: 1)
...
private extension PVSwitch {
    ...
    private func configureTrack(frame: CGRect) {
        ...
        let trackThickness = frame.size.height * 0.7
        ...
        self.track.backgroundColor = UIColor.green
//        self.track.layer.shadowOpacity = 0.5
//        self.track.layer.shadowOffset = CGSize(width: 0.0, height: -1)
...
private extension PVSwitch {
    ...
    @objc private func onTouchUpOutsideOrCanceled(button: UIButton, with event: UIEvent) {
        ...
        if newXOrigin > (frame.size.width - (self.switchThumb.frame.size.width ) / 2) {
            self.changeThumbState(on: true, animated: false, sendAction: true)
        } else {
            self.changeThumbState(on: false, animated: false, sendAction: true)
        }
    }
